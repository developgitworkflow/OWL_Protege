    ...
    useEffect(() => {
        ...
        edges.forEach(e => {
            ...
            if (lowerLabel === 'subclassof') {
                simLinks.push({ source: source.id, target: target.id, isArrow: true, role: 'subclass', isInferred, label: cleanLabel });
                return;
            }

            // 2. Instance Of (Dashed Line, Standard Arrow) - DIRECT LINK
            if (lowerLabel === 'type' || label === 'a') {
                simLinks.push({ source: source.id, target: target.id, isArrow: true, role: 'instance', isInferred, label: cleanLabel });
                return;
            }
            ...
            if (lowerLabel.includes('disjoint')) {
                ...
                simLinks.push({ source: source.id, target: disjointId, isArrow: false, role: 'disjoint_link', isInferred, label: cleanLabel });
                simLinks.push({ source: target.id, target: disjointId, isArrow: false, role: 'disjoint_link', isInferred, label: cleanLabel });
                return;
            }
        ...
        // Links
        const linkGroup = linkLayer.selectAll("g").data(simLinks).join("g");
        ...
        const linkPath = linkGroup.append("path")
            ...
            .attr("marker-end", (d: any) => {
                ...
                if (d.role === 'subclass') return "url(#arrow-sub)";
                return "url(#arrow-std)";
            })
            .attr("stroke-dasharray", (d: any) => (d.role === 'instance' || d.role === 'disjoint_link' || !d.isArrow || d.isInferred) ? "4,4" : "");

        const linkLabelGroup = linkGroup.append("g").style("display", (d: any) => d.role === 'subclass' ? 'none' : 'block');
        ...
        linkLabelGroup.append("text")
            .text((d: any) => (d.label || '').replace('owl:', ''))
        ...
        const node = nodeLayer.selectAll("g").data(simNodes).join("g")
