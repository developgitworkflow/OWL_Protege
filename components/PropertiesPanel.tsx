...
import { Trash2, Plus, X, Box, ArrowRight, MousePointerClick, ListOrdered, Quote, Link2, GitMerge, GitCommit, Split, Globe, Lock, Shield, Eye, BookOpen, Check, User, AlertOctagon, Tag, ArrowRightLeft, Sparkles, Command, AlertCircle, Layers, Settings, Database, ChevronDown, ChevronRight, List, Network, Maximize2 } from 'lucide-react';
...
const AccordionSection: React.FC<{ 
    ...
    defaultOpen?: boolean;
    onExpand?: () => void;
}> = ({ title, icon, children, defaultOpen = false, onExpand }) => {
    const [isOpen, setIsOpen] = useState(defaultOpen);

    return (
        <div className="border border-slate-800 rounded-lg bg-slate-900/50 overflow-hidden mb-3 shadow-sm">
            <div 
                className="w-full flex items-center justify-between px-4 py-3 bg-slate-800/50 hover:bg-slate-800 transition-colors cursor-pointer select-none group"
                onClick={() => setIsOpen(!isOpen)}
                onDoubleClick={(e) => { e.stopPropagation(); if(onExpand) onExpand(); }}
            >
...
const VisibilitySelector: React.FC<{ value: string; onChange: (val: string) => void }> = ({ value, onChange }) => {
    ...
    return (
        <div className="relative" ref={containerRef}>
            <button
                onClick={() => setIsOpen(!isOpen)}
                className={`p-2 rounded-md border border-slate-700 bg-slate-800 hover:border-slate-500 transition-colors ${current.color}`}
...
const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ selectedNode, selectedEdge, allNodes, onUpdateNode, onUpdateEdge, onDeleteNode, onDeleteEdge, onCreateIndividual, onNavigate, onClose }) => {
  ...
  const [expandedSection, setExpandedSection] = useState<'identity' | 'axioms' | 'properties' | 'instances' | 'annotations' | null>(null);
        ...
        <div ref={panelRef} className="w-full h-full overflow-y-auto flex flex-col font-sans text-sm text-slate-200 bg-slate-950">
            ...
            <div className="p-5 space-y-8 flex-1">
                <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            type="text"
                            className="w-full bg-slate-900 border border-slate-800 hover:border-slate-700 focus:border-blue-500 rounded p-3 text-sm font-mono text-slate-300 focus:ring-1 focus:ring-blue-500 outline-none transition-all placeholder-slate-700"
                            value={edgeLabel}
                            onChange={(e) => {
            ...
            <div className="mt-4 pt-4 border-t border-slate-800 px-5 pb-5">
                <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-red-950/30 hover:bg-red-900/50 text-red-400 border border-red-900/50 rounded-lg p-3 text-xs font-bold transition-colors"
                >
    ...
    <>
        ...
        <div className="p-4 space-y-2 flex-1">
            ...
            <AccordionSection 
                ...
                defaultOpen
                onExpand={() => setExpandedSection('identity')}
            >
                <div className="space-y-4">
                    <div className="space-y-1">
                        ...
                        <input
                            type="text"
                            className="w-full bg-slate-950 border border-slate-700 hover:border-slate-600 focus:border-blue-500 rounded p-3 text-sm font-medium text-slate-200 focus:ring-1 focus:ring-blue-500 outline-none transition-all placeholder-slate-700"
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            type="text"
                            className="w-full bg-slate-950 border border-slate-700 hover:border-slate-600 focus:border-blue-500 rounded p-3 text-sm font-mono text-slate-300 focus:ring-1 focus:ring-blue-500 outline-none transition-all placeholder-slate-700"
            ...
            {(isObjectProperty || isClassNode) && (
                <AccordionSection 
                    ...
                    defaultOpen
                    onExpand={() => setExpandedSection('axioms')}
                >
                    {isObjectProperty ? (
                                        ...
                                        <div className={`bg-slate-950 border border-slate-800 rounded-lg p-2 group hover:border-slate-700 transition-colors flex gap-2 items-start ${showSyntaxHelp === method.id ? 'ring-1 ring-purple-500/50 border-purple-500/30' : ''}`}>
                                            ...
                                            <div className="flex-1 flex flex-col gap-1.5">
                                                <div className="flex gap-2 items-center">
                                                    <select 
                                                        className="bg-slate-900 border border-slate-800 rounded px-2 py-1.5 text-xs text-purple-300 font-bold font-mono focus:outline-none focus:border-purple-500"
                                                        value={method.name}
            ...
            <AccordionSection 
                title={isObjectProperty ? "Characteristics" : "Data Properties"} 
                icon={isObjectProperty ? <Settings size={14} className="text-slate-400"/> : <Tag size={14} className="text-green-400"/>}
                onExpand={() => setExpandedSection('properties')}
            >
                {isObjectProperty ? (
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {localData.attributes.map(attr => (
                                <div key={attr.id} className="bg-slate-950 border border-slate-800 rounded px-2 py-2 flex justify-between items-center group hover:border-slate-600 transition-colors">
                                    <select 
                                        className="bg-transparent text-xs text-slate-300 outline-none w-full appearance-none cursor-pointer"
                                        value={attr.name}
                                    ...
                                    <div className="flex-1 flex flex-col gap-2">
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className={`p-1.5 rounded text-[10px] font-mono border ${attr.isDerived ? 'bg-blue-900/30 border-blue-500/50 text-blue-300' : 'bg-slate-900 border-slate-700 text-slate-600'}`}
                                            ...
                                            <input 
                                                className="bg-transparent text-sm text-slate-200 outline-none placeholder-slate-600 font-medium w-full py-1"
                                                value={attr.name}
                                                onChange={(e) => updateAttribute(attr.id, 'name', e.target.value)}
                                                placeholder="hasAge"
                                            />
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="relative flex-1">
                                                <input 
                                                    className="bg-slate-900/50 border border-slate-800 focus:border-blue-500/50 rounded px-2 py-1.5 text-xs text-blue-300 outline-none placeholder-slate-600 w-full font-mono transition-colors"
                                                    value={attr.type}
            ...
            {isClassNode && (
                <AccordionSection 
                    ...
                    onExpand={() => setExpandedSection('instances')}
                >
                    <div className="space-y-3">
                        <p className="text-[10px] text-slate-500">
                        ...
                        <div className="flex gap-2">
                            <input 
                                className="flex-1 bg-slate-950 border border-slate-800 rounded px-3 py-2.5 text-xs text-slate-200 outline-none focus:border-blue-500 placeholder-slate-600"
            ...
            <AccordionSection 
                title="Annotations" 
                icon={<Quote size={14} className="text-slate-400"/>}
                onExpand={() => setExpandedSection('annotations')}
            >
                <AnnotationManager 
        ...
        <div className="mt-4 pt-4 border-t border-slate-800 px-5 pb-5">
            <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-red-950/20 hover:bg-red-900/30 text-red-400 border border-red-900/30 hover:border-red-900/50 rounded-lg p-3 text-xs font-bold transition-all"
            >
                <AlertOctagon size={14} />
                Delete {displayType}
            </button>
